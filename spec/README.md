...
| 24  | E-mails — connexion & extraction | 🚧 | **Post-v1, partiellement construit.** Connexion IMAP + extraction batchée ✅ construites. **Extension écrite, rien de codé** : écran de validation `/resolve-emails` (revient sur "tout automatique", item par item accepté/rejeté avant écriture) + **Q&A sur les mails en chat** (`search_emails`/`read_email`, recherche IMAP live, fenêtre dédiée 90j, pas de persistance) |
