...
| 07  | Notes (vault)               | ✅ | Structure `alfred-*` ✅ ; skills/`.claude` retirés ✅ ; **frontmatter `project`/`participants` peuplés par l'ingestion** ✅ ; **regroupement par projet** (vue « Projets », dossiers virtuels) ✅ ; **fusion/renommage de projet** (`merge_projects`, clic droit → « Fusionner avec… ») ✅. *Rangement physique par projet = hors v1*. **Reste (post-v1) : suppression seule d'un projet (sans fusion)** |
