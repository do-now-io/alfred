...
| [x] | **Logo bouton d'enregistrement décalé (spec/03, feedback tests)** : l'asset `alfred-logo.png` est bien carré (2000×2000), mais son contenu ne l'est pas (portrait en haut, mot « ALFRED » en bas) — décalage visuel dans les cadres carrés. Recadrage **CSS** (zoom + `object-position` manuel sur le cercle du portrait, masque le mot-symbole illisible en petit) via un composant partagé `AlfredAvatar` (sidebar, onboarding, visite guidée, page de guidage) | CF |
...
| [ ] | **Internationalisation FR/EN (spec/21, feedback tests)** — gros chantier transverse : `app_language` (`fr`\|`en`) + **choix à l'installation** (étape 0 onboarding) + réglage Langue ; catalogues de traduction (aucune chaîne UI en dur, hook `t()`) ; messages d'erreur backend via codes ; **langue des sorties IA** paramétrable (≠ langue UI) ; défauts localisés (conseils captation, template contexte, contenu de démarrage) ; **découpler les sections `Todo.md`/titres contexte** de leur libellé (clés stables) pour ne pas casser le parsing | |
