...
| [~] | **Ingestion augmentée** (spec 17) : ingestion 2 temps (analyse → résolution groupée + réécoute segment → finalisation) + enrichissement auto « Appris automatiquement » dans `Contexte Alfred.md` — ✅ *backend fait* (`analyze_transcription`/`finalize_ingestion`, event `clarifications-ready`, flag `augmented_ingestion` off par défaut) ; ⬜ **écran de résolution + réécoute WAV à faire** | UC |
