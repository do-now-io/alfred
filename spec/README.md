...
| 02  | Calendar                    | 📝 | **Post-v1, spec réécrite, rien de codé.** Google Calendar uniquement (Apple Calendar hors scope — conflit avec le retrait de l'entitlement `apple-events`, spec/12). Contexte **actif** : nouvel onglet Agenda, brief enrichi, indice de détection de projet (spec/16b), nouvel outil de lecture dans le chat. Sync démarrage + 15 min |
...
| 24  | E-mails — connexion & extraction | ✅ | **Post-v1, construit.** IMAP générique (`async-imap`+TLS), fenêtre glissante (14j défaut) + dédup `Message-ID` (`processed_emails`), pas de note vault par mail (provenance texte non cliquable `✉️ objet (date)`), extraction batchée (1 appel Claude/batch, défaut 15 mails) → tâches + rattachement projet + contexte (réutilise le schéma `ContextAddition` spec/16b tel quel). Sync au lancement + bouton manuel Réglages. Pivot de la vision « projets unifiés » (ROADMAP Phase G) |
| 25  | Collaboration sur notes partagées | 📝 | **Post-v1, spec écrite, rien de codé.** Étend spec/18 : lien vue + **lien édition** distincts (édition anonyme, façon Google Docs), autosave + détection de collision par version (pas d'OT/CRDT temps réel), panneau commentaires séparé, notification au démarrage, rapatriement dans le vault **jamais automatique** (bouton explicite + garde-fou sur conflit local) |
