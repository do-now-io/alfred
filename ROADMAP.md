...
| [~] | **Source audio mixte par défaut (spec/03/11, feedback tests)** : `recording_source` défaut `mixed` (micro + système) ; les points de départ lisent la config au lieu de `"mic_only"` codé en dur ; **repli auto sur `mic_only`** si système indispo (macOS sans helper Swift) ; contexte à la voix reste `mic_only` | CF |
