...
| 07  | Notes (vault)               | ✅ | Structure `alfred-*` ✅ ; skills/`.claude` retirés ✅ ; **frontmatter `project`/`participants` peuplés par l'ingestion** ✅ ; **regroupement par projet** (vue « Projets », dossiers virtuels) ✅ ; **fusion/renommage/suppression de projet** (clic droit sur le groupe) ✅ ; **clic sur le titre du projet** ouvre sa note de contexte unique (spec/16b) ✅. *Rangement physique par projet = hors v1* |
...
| 16b | Contexte par projet          | ✅ | **Post-v1, construit.** Scinde le contexte appris entre global (`Contexte Alfred.md`) et par projet (`alfred-intelligence/<Projet>.md`, une note par projet, `type: context`, **6 sections** Aperçu/Personnes/Décisions/Événements/Tâches/Vocabulaire) ; nouveau champ "Projets concernés" sur `/resolve` ; fusion/renommage via `merge_projects` (spec/07) ; extraction e-mail (spec/24) reçoit la liste des projets connus pour éviter les doublons |
...
| 24  | E-mails — connexion & extraction | 🚧 | **Post-v1, partiellement construit.** Connexion IMAP + extraction batchée + écran de validation `/resolve-emails` (§5, item par item, `pending_email_reviews`) + Q&A sur les mails en chat (§6, `search_emails`/`read_email`) ✅ construits. **Extension écrite, rien de codé** : UI de connexion revue (§1.1) — **sélection du fournisseur d'abord** (Gmail/iCloud/Yahoo/Autre, host/port pré-remplis, instructions numérotées + liens directs vers les mots de passe d'application) — **Outlook exclu** (Microsoft coupe les mots de passe d'application au 30/04/2026, OAuth2 hors scope) |
| 25  | Collaboration sur notes partagées | 🚧 | **Post-v1, en cours.** Étend spec/18 : lien vue + **lien édition** distincts ✅ (migration `alfred-backend`), `WS /share/{edit_slug}/sync` (`yrs-axum`, doc Yjs en mémoire seedé depuis le markdown) ✅ — **pas encore** : snapshot Postgres périodique, front CodeMirror 6 + curseurs live, panneau commentaires, notification au démarrage, rapatriement dans le vault (**jamais automatique**) |
